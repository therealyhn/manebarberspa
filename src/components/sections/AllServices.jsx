    ...
    return (
        <div ref={sectionRef} className="w-full px-2 sm:px-4 md:px-8 lg:px-20 py-6 md:py-16 
            ...
            <SectionHeading
                ...
                wrapperClassName="mb-6 md:mb-16 w-full"
                subtitleClassName={`font-lato text-xs sm:text-sm md:text-md font-semibold
                 text-prime text-center mb-2 md:mb-4 ${isVisible ? 'animate__animated animate__fadeInUp' : 'opacity-0'
                    }`}
                titleClassName={`font-prata text-xl sm:text-2xl md:text-4xl font-semibold text-black text-center 
                    ${isVisible ? 'animate__animated animate__fadeInUp animate__delay-1s' : 'opacity-0'
                        }`}
                imageClassName={`w-24 sm:w-32 md:w-50 
                    ${isVisible ? 'animate__animated animate__fadeInUp animate__delay-1s' : 'opacity-0'
                        }`}
            />
            ...
            md:gap-6 lg:gap-8 w-full max-w-7xl">
                {services.map((service, index) => (
                    <ServiceCard
                        ...
                        image={service.image}
                        title={service.title}
                        wrapperClassName={`${isVisible ? `animate__animated animate__${service.animation} animate__delay-2s` : 'opacity-0'
                            }`}
                        imageClassName="w-28 sm:w-36 md:w-40 lg:w-48 h-28 sm:h-36 md:h-40 lg:h-48 object-cover rounded-full 
